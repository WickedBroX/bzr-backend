...
// Database query helper for raw SQL queries
...
app.get('/api/info', strictLimiter, cacheMiddleware(300), async (req, res) => {
  ...
  try {
    ...
    const params = {
      chainid: 137, // Polygon - where BZR token has the most transfers
      apikey: getNextApiKey(),
    };
    ...
    try {
      const emergencyResult = await dbQuery(
        `SELECT 
          token_name, token_symbol, token_decimals,
          total_supply, formatted_total_supply,
          circulating_supply, formatted_circulating_supply,
...
// Admin endpoint to force refresh token info
app.post('/admin/refresh-token-info', strictLimiter, async (req, res) => {
  console.log(`[${new Date().toISOString()}] Admin refresh token info requested`);
  
  // Simple authentication - check for admin password in header or body
  ...
  if (adminPassword !== expectedPassword) {
    console.warn('! Unauthorized admin refresh attempt');
    return res.status(401).json({ message: 'Unauthorized' });
  }

  try {
    // Trigger a force refresh by calling /api/info with force=true
    const response = await axios.get(`http://localhost:${PORT}/api/info?force=true`);
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to refresh token info',
      error: error.message,
    });
  }
});
